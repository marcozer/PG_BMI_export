...
def apply_volume_mode(df: pd.DataFrame, mode: str = "tertiles") -> pd.DataFrame:
    ...
    if mode in {"annual_threshold", "mipd_annual_threshold"}:
        ...
        if mode == "annual_threshold":
        ...
        else:  # mipd_annual_threshold: mean annual minimally invasive pancreatectomies = total / number of years observed
            df["NOMBRE_MIPD"] = pd.to_numeric(df.get("NOMBRE_MIPD"), errors="coerce")
            # assume NOMBRE_MIPD is a centre-level total over the dataset; take the first non-missing per centre
            mipd_total = (
            ...
            years_per_centre = (
                df.groupby("CENTRE", observed=False)["year"]
                .nunique()
...
def main(volume_tier_mode: str = "tertiles") -> None:
    ...
    # record mode used for plotting reproducibility
