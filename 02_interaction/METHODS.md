...
- Courbe par tiers de volume : prédictions du GLM le long de 18–45 kg/m², médiane de volume par tier (Low/Mid/High), IC 95% par delta‑method avec covariance cluster‑robuste. Fichiers : `bmi_volume_curves.csv`, `bmi_interaction_tiers.svg/png`.
...
- Volume-tier curves: GLM predictions across BMI 18–45 at median volume per tier (Low/Mid/High) with 95% CIs via delta method using the model’s cluster‑robust covariance. Files: `bmi_volume_curves.csv`, `bmi_interaction_tiers.svg/png`.
